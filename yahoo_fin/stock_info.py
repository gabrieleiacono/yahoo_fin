...
def tickers_ftsemib():
    ...
    site = "https://en.wikipedia.org/wiki/FTSE_MIB"

    table = pd.read_html(site, attrs = {"id":"constituents"}, header=0)[0]

    ftsemib_tickers = sorted((table['Ticker'] + '.MI').tolist())

    return ftsemib_tickers

def tickers_dax():
    '''Downloads list of currently traded tickers on the DAX index'''

    site = "https://en.wikipedia.org/wiki/DAX"
...
def tickers_cac():
    '''Downloads list of currently traded tickers on the CAC40 Index'''

    site = "https://en.wikipedia.org/wiki/CAC_40"

    table = pd.read_html(site, attrs = {"id":"constituents"}, header=0)[0]

    cac_tickers = sorted(table['Ticker'].tolist())
...
@retry(wait_random_min=400, wait_random_max=1400)
def get_stats(ticker, headers = {'User-agent': 'Mozilla/5.0'}):

    '''Scrapes information from the statistics tab on Yahoo Finance
       for an input ticker.
       It also includes valuation data, making get_stats_valuation returning a subset of these data.
